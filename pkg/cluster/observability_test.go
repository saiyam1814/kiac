...
func TestObservabilityManifests(t *testing.T) {
	cases := []struct {
		...
		{"grafana", obsGrafanaManifest, []string{"docker.io/grafana/grafana", "type: LoadBalancer", "ipFamilyPolicy: PreferDualStack", "GF_AUTH_ANONYMOUS_ENABLED", "path: /api/health"}},
	}
